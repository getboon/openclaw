...
/**
 * An optional local-time active-hours window (start/end HH:MM + timezone), with
 * self-contained HH:MM validation. Shared by HeartbeatSchema and
 * ProgressNudgeSchema so the 24:00 edge-case rules stay in one place.
 */
...
export const HeartbeatSchema = z
  ...
  .superRefine((val, ctx) => {
    ...
    // activeHours HH:MM validation is owned by ActiveHoursSchema (the field type).
  })
  .optional();
