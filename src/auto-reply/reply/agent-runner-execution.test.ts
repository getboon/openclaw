...
describe("runAgentTurnWithFallback", () => {
  ...
  it.each(NON_DIRECT_FAILURE_SURFACE_CASES)(
    ...
    async (testCase) => {
      ...
      if (result.kind === "final") {
        ...
        expect(result.payload.text).not.toBe(SILENT_REPLY_TOKEN);
        // Fork (#28): a bare 429 with no fallback summary and no known cooldown
        // is treated as an unexplained transient edge block, so the honest
        // "briefly busy" copy is surfaced instead of "rate-limited".
        expect(result.payload.text).toContain("briefly busy");
      }
    },
  );
  ...
  it("surfaces transient-busy fallback copy in Discord group chats when silentReply.group is disallow", async () => {
    state.runEmbeddedAgentMock.mockRejectedValueOnce(new Error("429 rate limit exceeded"));
    ...
    if (result.kind === "final") {
      expect(result.payload.isError).toBe(true);
      expect(result.payload.text).not.toBe(SILENT_REPLY_TOKEN);
      // Fork (#28): bare 429 → honest "briefly busy" copy, not "rate-limited".
      expect(result.payload.text).toContain("briefly busy");
    }
  });
