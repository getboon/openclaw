...
// App_mention + message events for one Slack message can miss each other's
// debounce window and flush independently, racing on session init (see
// src/auto-reply/reply/session.ts). isReplySessionInitializationConflictError
// classifies that known-transient race so it can be retried like Telegram does.
      ...
      const retryEntries = (sourceError: unknown): boolean => {
        ...
        const nextEntries = entries
          .map((entry) => {
            ...
            // Scheduling here too can race the router redelivery and duplicate a reply.
