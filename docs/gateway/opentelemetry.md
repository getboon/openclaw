...
- `openclaw.model.failover` (counter, attrs: `openclaw.provider`, `openclaw.model`, `openclaw.failover.to_provider`, `openclaw.failover.to_model`, `openclaw.failover.reason`, `openclaw.failover.outcome`, `openclaw.failover.tier`, `openclaw.failover.suspended`, `openclaw.lane`). `outcome` is one of `next_fallback`, `chain_exhausted`, `succeeded`, or `suspend_lanes`; `tier` is the 1-based ladder position. The `chain_exhausted` series is the client-side signal for the operator "fallback chain exhausted" alert.
