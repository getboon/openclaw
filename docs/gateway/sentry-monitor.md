...
| `before_tool_call_hook_failed` | a genuine `before_tool_call` handler exception (`kind: "failure"`, never a policy veto), when this hook is enabled | `hook`, `host`, `tool`                                                                   |
...
`before_tool_call_hook_failed` is the counterpart to `after_tool_call` for the pre-execution hook pipeline: the host fires it only when a `before_tool_call` handler throws an unhandled exception (an internal `kind: "failure"` block), never for a deliberate policy veto (loop-breaker, approval-required, plugin `block: true`). Because every event that reaches it is a genuine defect, it is always captured as an exception with no `denied`-style suppression, and it is fingerprinted by `tool` + the normalized error so repeats of the same failure bucket into one issue.
