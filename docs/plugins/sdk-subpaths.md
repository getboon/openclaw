...
usage endpoint failed or returned no usable usage data.
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/browser-profile-config` | Register or remove a remote-CDP browser profile owned by the bundled browser plugin, for sibling plugins that hand off an attach-only session |
    ...
    | `plugin-sdk/gateway-activity-runtime` | `getGatewayActiveRunCount`, the live unaborted-run count for the in-process Gateway, or `null` when no Gateway is running; use it for drain checks before restart-style actions |
