...
- `channels.msteams.alwaysFetchGraphMessage`: always re-fetch the channel/group message via Microsoft Graph (in addition to the inbound Bot Framework activity) when the local media list is empty. Workaround for tenants where Bot Framework strips file refs (`<attachment id=...>` tags and `reference`-typed entries) from inbound activities even with RSC consent. Default: `false`. Costs one extra Graph round-trip per channel/group message with empty media; personal-chat conversations are unaffected.
