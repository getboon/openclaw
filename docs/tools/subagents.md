...
- **Run timeout:** OpenClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise sub-agents inherit the general `agents.defaults.timeoutSeconds` run timeout. Set `runTimeoutSeconds: 0` explicitly for no timeout. `sessions_spawn` does not accept per-call timeout overrides.
